...
Esta guía define el sistema visual, de copy y técnico para **bistro.example.com**. Adapta el lenguaje de la **Fase 2 del rediseño** del sitio marketing (`funcionbase.com` v2.1) a una aplicación SaaS operativa: POS, caja, planificador de turnos, reportes, PWA, modo offline.
...
| Aspecto | `funcionbase.com` | `bistro.example.com` |
|---|---|---|
...
8. **Labels de categoría sobre surface dark** — `text-accent` uppercase tracking sobre `bg-foreground` para etiquetar grupos en footer dark, sidebar dark (cuando está activo), panel oscuro de configuración. Patrón validado en footer de `funcionbase.com/` (`EXPLORAR`, `EMPRESA`).
...
Mismo color (`#C0FD79`), **misma filosofía**. Una inspección visual del website `funcionbase.com` realizada en mayo 2026 confirma que app y website convergen en el uso del lime:

**Patrón observado en el website 2026** (home `funcionbase.com/`):
...
- **Header**: logo + razón social + NIT + sede. Usar `FlexyFont` para "FUNCIONBASE" si aparece como wordmark.
