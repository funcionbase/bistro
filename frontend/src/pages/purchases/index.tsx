...
export default function PurchasesIndex() {
    ...
    const blocked = suppliers.suppliers.length === 0 || inventory.ingredients.length === 0;
        ...
        <PageShell title="Compras">
            <div className="flex flex-col gap-6 p-4 lg:p-6">
                <PageHeader
                    ...
                    description="Borradores → confirmadas → recibidas (mueven inventario) → pagadas. La anulación post-recepción genera nota crédito + reverso."
                    actions={
                        <>
                ...
                {blocked && (
                    <Alert variant="warning">
                        ...
                        <AlertDescription>
                            Para crear órdenes necesitas {suppliers.suppliers.length === 0 ? 'al menos un proveedor activo' : ''}
                            {suppliers.suppliers.length === 0 && inventory.ingredients.length === 0 ? ' y ' : ''}
                            {inventory.ingredients.length === 0 ? 'al menos un insumo activo' : ''}.
                        </AlertDescription>
                    </Alert>
                )}
                        ...
                        <EmptyState
                            ...
                            description={
                                hasActiveFilters
                                    ...
                                    : 'Crea una orden para empezar a registrar compras a proveedores; al recibirla se actualiza el inventario.'
                            }
                            action={
                                !hasActiveFilters &&
                                !blocked && (
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {purchases.orders.map((p) => (
                                    <TableRow key={p.id} className="hover:bg-muted/40 cursor-pointer transition-colors" onClick={() => openDetail(p)}>
