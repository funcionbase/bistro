...
import { AlertCircle, Archive, Pencil, Plus, RefreshCw, RotateCcw, Truck } from 'lucide-react';
    ...
    return (
        <PageShell title="Proveedores">
            <div className="flex flex-col gap-6 p-4 lg:p-6">
                {sup.loading && sup.suppliers.length === 0 ? (
                    <SuppliersListSkeleton rows={5} />
                ) : (
                    <>
                        <PageHeader
                            eyebrow="CATÁLOGO"
                            title="Proveedores"
                            description="Catálogo de proveedores con datos de contacto, documento tributario y términos de pago."
                            actions={
                                <>
                                    <Button variant="outline" onClick={() => sup.fetchSuppliers()} disabled={sup.loading}>
                                        <RefreshCw className={`mr-1 h-4 w-4 ${sup.loading ? 'animate-spin' : ''}`} />
                                        Refrescar
                                    </Button>
                                    <Button
                                        onClick={() => {
