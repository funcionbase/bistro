...
| Tabla | Campos clave |
|-------|--------------|
| `companies` | `id` (UUID v7, PK), `nit` (UNIQUE, inmutable), `commercial_name`, `legal_name`, `bank_id`, `account_number`, `account_type` (`ahorros`/`corriente`), `breb_key`, `qr_code_path`, `logo_path`, `status` (`pending_activation`/`active`/`past_due`/`suspended`/`rejected`/`inactive`), `tax_regime`, `default_tax_rate`, `default_tax_label`, `tax_included_in_price`, `past_due_started_at`, `expected_block_at`, `payment_blocked_at`, `activation_notified_at` (#257), perfil fiscal DIAN (`dv`, `legal_representative_*`, `economic_activity_code`, `fiscal_responsibilities`, `tax_obligations`, `municipality_dane_code`, `billing_email`, `billing_phone`, `physical_address`, `country_code`). El campo `plan` fue eliminado en #257 — vive en `subscriptions` con snapshot inmutable. |
