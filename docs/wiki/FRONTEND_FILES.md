...
| Archivo | URL | Permiso (gate web) | APIs | Notas |
|---------|-----|---------------------|------|-------|
| `orders/board.tsx` | `/orders/board` | `orders.read` (vía API en hooks) | `GET /api/v1/orders` (5s), `PATCH /api/v1/orders/{id}/status`, `POST /api/v1/deliveries/{id}/reassign`, `POST /api/v1/orders/{id}/assign-courier` | Tablero kanban 5 columnas: Pendientes, En Cocina, Para Entrega, En Domicilio, Completado. Drag-and-drop entre columnas (dnd-kit). Animación drop-bounce. Mobile: select dropdown de columnas. Hooks: `useOrders`, `useCourierAssignment`. Cálculo de total: server-side; el frontend no inyecta precios |
...
| `whatsapp.tsx` | `/company/whatsapp` | `whatsapp.read` (gate web) | `GET /api/v1/whatsapp`, `POST /api/v1/whatsapp/embedded-signup-callback`, `POST /api/v1/whatsapp/naas-request`, `DELETE /api/v1/whatsapp/phone`, `DELETE /api/v1/whatsapp`, `POST /api/v1/whatsapp/verification/{request,verify}`, `GET/PATCH /api/v1/companies/settings` | Estados: sin conexión (Embedded Signup vs NaaS) / conectado (info número + Cambiar/Desconectar). Modal OTP `WhatsappVerificationCodeModal` antes de cada acción sensible. Bloque "Preferencias" con privacidad (`whatsapp_read_receipts`) y bot (`bot_welcome_message`, `bot_away_message`). Carga SDK de Facebook |
...
| `chats.tsx` | `/chats` | `chats.read` | `GET /api/v1/chats` (5s), `GET /api/v1/chats/{id}`, `POST /api/v1/chats/{id}/messages`, `POST /api/v1/chats/{id}/mark-read`, `GET /api/v1/chats/{id}/client` | Panel 2 columnas: lista (5s polling) + conversación. Búsqueda con `?q=` (debounce 300ms). `ChatSourceBadge` (whatsapp/instagram/facebook). Última orden clickeable abre `OrderDetailModal`. `ClientDetailModal` con historial. Mark-read despacha al backend solo si chat abierto + tab visible + hay `meta_message_id`. Reproductor audio (`AudioPlayer`) estilo WhatsApp |
...
| Archivo | URL | Permiso | APIs | Notas |
|---------|-----|---------|------|-------|
| `clients/index.tsx` | `/clients` | `clients.read` | `GET /api/v1/clients?search=&segment=&tag=&page=&per_page=` | Listado con búsqueda (debounce 300ms), filtros por segmento (`vip`, `recurrent`, `new`, `inactive`, `at_risk`, `regular`) y por tag. Tabla con teléfono, nombre, pedidos, ticket promedio, total gastado, última orden, segmento, tags. Paginación 25/página. Phone display amigable (`+57 3XX XXX XXXX`) — en API/BD siempre `573XXXXXXXXXX`. |
| `clients/show.tsx` | `/clients/{phone}` | `clients.read` | `GET /api/v1/clients/{phone}`, `POST/DELETE .../notes`, `POST/DELETE .../tags`, fidelización via `LoyaltyPanel` (#122) | Perfil consolidado cross-sede. Header con nombre + phone + segmento + 8 KPIs (órdenes totales, total gastado, ticket promedio, primera/última orden, órdenes 60d, gasto 90d, % cancelaciones). `TagsEditor` chips agregar/quitar. **Panel de fidelización** (`<LoyaltyPanel>`) entre header y tabs si tiene `loyalty.read`. Tabs: historial de órdenes (50 últimas, link a chat si tiene), chats (cross-sede), notas privadas (`NotesPanel`, soft-delete con confirmación). Botón "Ver chat" si existe al menos una conversación. |
