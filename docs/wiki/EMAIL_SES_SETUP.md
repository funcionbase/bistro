...
| `MAIL_FROM_ADDRESS`       | `noreply@...`      | `hello@funcionbase.com`                    | Debe pertenecer al dominio verificado en SES.      |
| `MAIL_FROM_NAME`          | `${APP_NAME}`      | `${APP_NAME}`                             | Hereda del nombre de la app.                       |
| `MAIL_REPLY_TO_ADDRESS`   | `soporte@...`      | `hello@funcionbase.com`                    | Buzon real (Cloudflare Routing → Gmail).           |
...
SPF para el dominio raíz (cuando enviás desde `hello@funcionbase.com`):
...
`FromAddress=hello@funcionbase.com` — si después agregás otros remitentes
...
```
MAIL_MAILER=ses
MAIL_FROM_ADDRESS=hello@funcionbase.com
MAIL_REPLY_TO_ADDRESS=hello@funcionbase.com
```
...
entorno (`noreply@qa.example.com` vs `hello@funcionbase.com`), ajustar
...
php artisan config:show mail.from.address   # hello@funcionbase.com
php artisan config:show mail.reply_to       # array con hello@funcionbase.com
