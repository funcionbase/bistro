...
- **Cierre de Caja**: endpoint `GET /api/v1/reports/cash-drawer` retorna ingresos/devoluciones/propinas por método de pago en un rango (default: hoy en TZ Bogotá). Calcula `cash_drawer_expected = cash_gross + cash_tips - cash_refunds` para conciliación física. PDF exportable via `cash-drawer/pdf`. UI en `Mi Empresa › Informes` (`CashDrawerCard`). Filtra por `payment_receipts.paid_at` (no `orders.ordered_at`) para reflejar la fecha real del movimiento de dinero.
...
| `app/Http/Controllers/Api/OrderController.php` | `updateStatus` ahora aplica regla forward-only via `assertForwardOnlyTransition()` y descuenta inventario una vez al pasar a `in_kitchen` (idempotencia con `inventory_consumed_at`). `appendItems` posterior al consumo descuenta solo el delta. Helper privado `consumeInventoryForItems()` agrega cantidades por `menu_item_id`, recorre recetas activas, convierte unidades y registra `sale_consumption` con `allowNegativeStock=true`. |
