...
| Mail | log (local) / ses (qa/pdn) | Amazon SES via IAM instance profile; templates `vendor/mail/html` personalizadas con branding bistro. Ver [`EMAIL_SES_SETUP.md`](EMAIL_SES_SETUP.md) |
...
| `JWT_COOKIE_NAME` | `bistro_jwt` | Nombre de la cookie HttpOnly |

### Cookie de JWT

- **Nombre:** `bistro_jwt` (excluida de Laravel `EncryptCookies` porque ya viene cifrada).
...
- **Pedido público sin mesa (QR de sede)**: `Public\BranchOrderController::store` (`POST /api/v1/public/branch/{menu_qr_token}/orders`, throttle `branch-order-public` 5/min IP+token, FormRequest `Public\StoreBranchOrderRequest` con trait `SanitizesInput`). Crea `Order` `order_type=pickup|delivery`, `status=pending_approval`, items `order_items` con precio del menú activo de la sede; el envío entra como línea sintética `menu_item_id='delivery_fee'` (constante `Order::DELIVERY_FEE_ITEM_ID`, "Domicilio", tax 0) con el precio de `branch_settings.delivery_fee` (editable vía `BranchController::updateSettings`). Totales via `OrderTotalCalculator`. **Paridad caja**: `OrderController::store` inyecta la misma línea para `order_type=delivery` antes del aggregate (prorrateo de cupón idéntico); en `materializeOrderItems` la fila del fee nace `served` (no aparece en KDS ni bloquea `ready`). `BootstrapService` expone `activeBranch.delivery_fee` para el preview de total en caja. Contact upsert por phone (CRM). Aprobación staff: `OrderController::approve` (`POST /api/v1/orders/{id}/approve`, `orders.update`, solo órdenes sin `table_session_id`) → orden a `pending`, items a `approved`, audit `order.approved` + SMS dispatcher. Los pedidos aparecen en `TableSessionController::pendingApprovals` (path 2 extendido a `pickup|delivery` con `client_name/client_phone/delivery_address/total`). Push a staff reutiliza `OrderItemSubmittedForApproval`. **Sesión de carta (F3)**: `Public\CartSessionController` (throttle `cart-public`) — `POST cart/{token}/activity` (tracking `last_activity_at`, siempre 204), `GET cart/{token}/orders` (estado público CA6 con labels derivados), `POST cart/{token}/orders/{order}/items` (append del cliente mientras `pending_approval`, re-chequeo bajo lock → 409 `ORDER_ALREADY_APPROVED`, audit `order.items_appended_by_customer`, ChatMessage bot). `orders.cart_session_id` liga multi-orden; `cart_sessions.viewed_at/last_activity_at` alimentan el panel del chat. **Recibo térmico virtual (F4)**: `App\Services\WhatsappReceiptBuilder` (32 cols, triple backtick, total desde `orders.total`, strip ESC/GS/backticks — NO reemplaza al fiscal `Printing\DianReceiptBuilder`); `ChatController::sendReceipt` (`POST /chats/{id}/orders/{orderId}/receipt`, guards bajo lock `RECEIPT_ALREADY_SENT`/`ORDER_CHANGED` con `orders.receipt_sent_at/receipt_sent_total`) y `ChatController::rejectProof`; `OrderController::approve` acepta `expected_total` (409 `ORDER_CHANGED`); `ChatController@show` adjunta `cart_flow` (sesión + órdenes con guard de recibo) al `ChatResource`.
...
**Abandono de pedidos públicos** — `MarkAbandonedOrdersCommand` (`orders:mark-abandoned`, hourly + `onOneServer`): pedidos QR de sede sin sesión de mesa que llevan más de `config('orders.abandon_after_hours')` (24h) en `pending_approval` pasan a `abandoned` (métrica de carritos perdidos — antes ningún código asignaba ese estado) con items cancelados (`system`) y audit `order.abandoned`.
...
| `ChatController` | `index, show, storeMessage, markRead, clientDetail, updateBot, updateContact, menuLink` | Operador ↔ cliente WhatsApp. `menuLink` (antes `cartLink` con CartJWT a pedidos.example.com): crea `CartSession` ligada al chat y devuelve token para el link corto `/menus?cart={uuid}` |
| `MunicipalityController` | `index` | Catálogo DANE para el selector de ciudad (`?q=` búsqueda / `?code=` resolución). Global, sin scope de empresa. Tabla `municipalities` poblada desde CSV en migración |
...
| `DeliveryService` | Asignar, reasignar, completar; cálculo de duración; métricas. **Gate `assertOrderAssignable`**: solo domicilios (`order_type` delivery/null) en `in_kitchen`/`ready` admiten repartidor (mismo set que `available()`) — sin esto un cliente API saltaba cocina (`pending→in_transit`) o ponía una mesa en tránsito. Coherencia con KDS: `assignDeliverer`/`selfAssign` sirven los items abiertos vía `OrderItem::serveOpenItems` SOLO si la orden estaba `ready` (si el courier la tomó con cocina trabajando, los tickets se conservan); `completeDelivery` (→`completed`) sirve lo que quede; `rejectDelivery` cancela la orden Y cierra sus items vía `OrderItem::cancelOpenItems` (mismo cierre que `OrderController::cancel`) | `config/delivery.php` |
...
| `JWT_COOKIE_NAME` | bistro_jwt | Nombre cookie HttpOnly |
...
| `CART_BASE_URL` | https://pedidos.example.com | URL pública del frontend de carrito |
...
| `app/Http/Controllers/Api/OrderController.php` | `updateStatus` aplica regla forward-only via `assertForwardOnlyTransition()` y descuenta inventario una vez en cualquier avance a rank >= `in_kitchen` (drags que saltan cocina incluidos; idempotencia con `inventory_consumed_at`, referencia `order.<target>`). `appendItems` posterior al consumo descuenta solo el delta. **Vender completado descuenta stock**: los cierres que no pasan por el kanban también consumen vía `InventoryService::consumeForOrderOnce` (silencioso, nunca bloquea el cobro) — `closeWithPayment`, `TableCashierService::maybeCloseSession`, `SyncController`/`OrderSyncController` (cierres offline) y la promoción KDS. **Gate de pago en `completed`**: mesa/pickup exigen `PaymentReceipt` no-refund para completarse (`0338d61`); domicilio se exceptúa (contra-entrega). **Gate `in_transit`**: `updateStatus()` rechaza `in_transit` para órdenes `table`/`pickup` (422) — solo domicilios transitan (v1.30.3). **Delivery fee en caja** (#whatsapp F1): `store()` anexa la línea sintética `delivery_fee` (`Order::DELIVERY_FEE_ITEM_ID`, tax 0, `served`) leyendo `branch_settings.delivery_fee` cuando `order_type='delivery'`, antes del `aggregate()` para que el prorrateo de cupón la incluya; paridad con `BranchOrderController`. |
...
|---|---|---|---|---|
| `GET` | `/manifest.webmanifest` (`pwa.manifest`) | `App\Http\Controllers\PwaManifestController@show` | Pública (lee JWT cookie si está) | Sirve el Web App Manifest. Si hay JWT válido con `active_company_nit`, el manifest usa `commercial_name` y `menu_primary_color` de esa empresa; si no, devuelve branding bistro por defecto. |
...
- Fallback estático sin lógica: `public/manifest.webmanifest` (bistro brand). Sirve como red de seguridad si la ruta dinámica falla.
...
| `GET` | `/apple-touch-icon.png` (`pwa.apple-touch-icon`) | `App\Http\Controllers\PwaManifestController@appleTouchIcon` | Pública | Sirve el apple-touch-icon de la empresa activa (rasterizado desde su logo) o el bistro black-font por defecto. |
...
- Workflow `.github/workflows/ops-app-deploy.yml` corre el migrate en un step previo (`Run migrations (single instance)`) contra una sola EC2 InService, vía SSM, con timeout 600s. Si falla, aborta antes de tocar el resto de nodos.
