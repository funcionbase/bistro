...
| `OrderTotalCalculator` (`App\Support`) | `computeForOrder($order)` y `recalculateAndSave($order)`. Recalcula `subtotal/tax_amount/tax_rate/total` desde filas `order_items` no-canceladas delegando en `TaxCalculator` + snapshot tributario de la orden (`order_items.tax_rate` por línea, fallback `snapshot_default_tax_rate`) — consolidación caja/QR (#293). Prorratea descuento de cupón igual que `appendItems`. Caller debe tener `lockForUpdate`. | — |
