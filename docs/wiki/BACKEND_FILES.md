...
| `DeliveryService` | Asignar, reasignar, completar; cálculo de duración; métricas. Coherencia con KDS: `assignDeliverer`/`selfAssign` (→`in_transit`) y `completeDelivery` (→`completed`) sirven los items abiertos vía `OrderItem::serveOpenItems` (sin esto quedaban tickets fantasma en el KDS — el `updateStatus` posterior del frontend es no-op y nunca corre `syncItemsToOrderStatus`); `rejectDelivery` cancela la orden Y cierra sus items vía `OrderItem::cancelOpenItems` (mismo cierre que `OrderController::cancel`) | `config/delivery.php` |
