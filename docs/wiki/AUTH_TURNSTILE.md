...
| Secret key | GH Environment `pdn` → secret `TURNSTILE_SECRET_KEY` → `sync-env-secret.yml` → AWS Secrets Manager `flexyflow-bistro/pdn/dotenv` → `.env` de la EC2 | Nunca en el frontend ni commiteada. |

Widget en Cloudflare: **Turnstile → `bistro-auth`** (hostname `bistro.flexyflow.co`, modo Managed). Para rotar claves: regenerar en el dashboard, actualizar `.env.production` (rebuild+deploy frontend) y el secret `TURNSTILE_SECRET_KEY` (re-sync + App Deploy backend).
