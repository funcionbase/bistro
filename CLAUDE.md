...
Features con jobs/crons/cache/sesiones/storage deben ser N-instance safe (la app corre en un ASG con N instancias): `->onOneServer()` en schedules, driver compartido (`redis`/`database`) para cache/sesiones/colas, storage en `s3`. Detalle en `apps-flexyflow-co/.claude/infra-aws.md`.
