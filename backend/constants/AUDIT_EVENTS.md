...
| `order.created_by_customer` | `Public\BranchOrderController::store` (pedido sin mesa por QR de sede) | `company_nit`, `branch_id`, `order_type` (pickup/delivery), `client_phone`, `items_count`, `delivery_fee`, `total`, `payment_preference`, `tip_amount` | Público sin JWT — sin actor. |
| `order.items_appended_by_customer` | `Public\CartSessionController::appendItems` (append desde la carta, F3) | `company_nit`, `cart_session_id`, `items_count`, `total` | Público sin JWT — sin actor. Solo órdenes `pending_approval` de la misma sesión. |
| `order.approved` | `OrderController::approve` (pedidos públicos sin mesa) | `order_id`, `order_type`, `total` | `orders.update` |
| `order.abandoned` | `MarkAbandonedOrdersCommand` (cron `orders:mark-abandoned`) | `order_id`, `order_type`, `ordered_at`, `total` | Sin actor (system action — cron). |
