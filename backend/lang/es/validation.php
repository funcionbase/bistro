...
|--------------------------------------------------------------------------
|
| Traducción ES de los mensajes de validación de Laravel. Sin este archivo
| el validador devuelve la clave cruda (`validation.required`, etc.) porque
| APP_LOCALE=es y no había carpeta `lang/`. Los `messages()` de cada
| FormRequest siguen teniendo prioridad sobre estas líneas.
|
*/
...
return [
    ...
    'password' => [
        ...
        'uncompromised' => 'La contraseña indicada se ha filtrado en una fuga de datos. Elige una contraseña distinta.',
    ],
    ...
    |--------------------------------------------------------------------------
    |
    | Convención `'campo.regla' => 'mensaje'` para sobrescribir un mensaje
    | puntual. Los `messages()` de cada FormRequest cubren la mayoría de
    | estos casos; este bloque queda como fallback global.
    |
    */
    ...
    |--------------------------------------------------------------------------
    |
    | Traduce el placeholder `:attribute` a una etiqueta legible. Si un campo
    | no está aquí, Laravel usa el nombre con guiones bajos como espacios.
    |
    */
