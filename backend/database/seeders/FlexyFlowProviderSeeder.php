 ...
 * `BISTRO_NIT`, igual NO la crea — emite warn y skipea. Para tener una
 * Company bistro en local, usar el enrollment regular.
 *
...
class funcionbaseProviderSeeder extends Seeder
{
    public function run(): void
    {
        ...
        if ($company === null) {
            $this->command?->warn(
                "bistro Company NIT={$nit} no existe — skip resoluciones DIAN. ".
        ...
        $this->command?->info("bistro SaaS provider: empresa {$company->nit} + resoluciones + MockProvider asegurados.");
    }
    ...
    private function ensureProviderConfig(string $nit): void
    {
        DianProviderConfig::query()->firstOrCreate(
            ...
            [
                ...
                'software_id' => 'BISTRO-SAAS-'.Str::upper(Str::random(8)),
                'software_pin_encrypted' => Str::random(16),
                'test_set_id' => 'BISTRO-TESTSET-001',
                'environment' => 'habilitacion',
